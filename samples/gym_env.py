...
class GymEnviroment(BasicEnviroment):
    def __init__(self, type, name, render_mode=None):
        ...
        self.obs_space_info = self.parseEnvSpace(self.env.observation_space)
        self.action_space_info = self.parseEnvSpace(self.env.action_space)
        self.reset()
        self.Done = False

    def parseEnvSpace(self, space):
        space_info = {}
    ...
    def step(self, action):
        ...
        self.obs = next_obs
        return obs, self.reward, next_obs, self.done, self.info
